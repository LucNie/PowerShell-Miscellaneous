...
$confirm = "N"
$confirm = Read-Host 'Node modules ? Install(I) Install-Force(IF) Delete(D) Update(U)'
...
elseif ($confirm -eq "IF") {
    For ($i = 0; $i -lt $arr.length ; $i++) { 
        Write-Output "node-modules install (FORCED): $($arr[$i])" 
        start-process powershell.exe -argument "-nologo -noprofile -command cd ./'$($arr[$i])' ; npm i --force ;exit"
    }
}
elseif ($confirm -eq "D") {
    For ($i = 0; $i -lt $arr.length ; $i++) { 
